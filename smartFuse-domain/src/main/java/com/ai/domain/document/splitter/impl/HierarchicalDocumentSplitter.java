...
import static com.ai.common.util.Utils.firstChars;
        ...
        for (String part : parts) {
            if (segmentBuilder.hasSpaceFor(part)) {
                ...
                if (segmentBuilder.hasSpaceFor(part)) {
                    ...
                    if (subSplitter == null) {
                        throw new RuntimeException(format("The text \"%s...\" (%s %s long) doesn't fit into the maximum segment size (%s %s), " + "and there is no subSplitter defined to split it further.", firstChars(part, 30), sizeOf(part), tokenizer == null ? "characters" : "tokens", maxSegmentSize, tokenizer == null ? "characters" : "tokens"
                        ));
                    }
                    segmentBuilder.append(part);
